...
@section('container')
    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
      <h1 class="h2">Postingan Jurnalistik</h1>
    </div>
        ...
        <table class="table table-striped table-sm">
          ...
          @foreach ($jurnalistik as $jurnal)
            <tr>
              ...
              <td>{{ $jurnal->judul }}</td>
              <td>{{ $jurnal->categoryJurnalistik->name }}</td>
              <td>
                <a href="/dashboard/jurnalistik/{{ $jurnal->slug }}" class="badge bg-info">
                    <span data-feather="eye"></span>
                </a>
                <a href="/dashboard/jurnalistik/{{ $jurnal->slug }}/edit" class="badge bg-warning">
                    <span data-feather="edit"></span>
                </a>
                <form action="/dashboard/jurnalistik/{{ $jurnal->slug }}" method="post" class="d-inline">
                  @method('delete')
                  @csrf
